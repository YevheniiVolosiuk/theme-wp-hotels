...
class Assets
{
  ...
  public function modifyScriptTag($tag, $handle, $src)
  {
    ...
    switch ($handle) {
      case 'front-page':
      case 'main-js':
        $tag = '<script type="module" crossorigin  src="' . esc_url($src) . '" id="' . esc_attr($handle) . '"></script>' . PHP_EOL;
        break;
      default:
        $tag = '<script type="text/javascript"  src="' . esc_url($src) . '" id="' . esc_attr($handle) . '"></script>' . PHP_EOL;;
        break;
    }
